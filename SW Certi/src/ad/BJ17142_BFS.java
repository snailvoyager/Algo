	...
	public static void bfs(Node[] v) {
		...
		while(!Q.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(xx>=0 && xx<N && yy>=0 && yy<N && Map[xx][yy] != -1 && !Visit2[xx][yy]) {	//비활성 바이러스 칸도 방문해야한다
					if(Map[xx][yy] == 0)
						Cnt++;				//바이러스가 퍼진 카운트는 빈칸 수만
		...
		for(Node i : Virus) {
			Map[i.x][i.y] = -2;			//바이러스 자리 원복. 비활성 바이러스 칸에서 등록된 시간은 무의미해진다
		}
	}
